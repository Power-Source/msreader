...
class WMD_MSReader {
	...
	function post_indexer_notice() {
		...
		if ((($page === 'msreader.php' && $pagenow == 'settings.php') || $pagenow == 'plugins.php') && is_super_admin() ) {
			// Prüfe ob das Multisite Index Plugin installiert ist
			$plugin_dir = WP_PLUGIN_DIR . '/ps-postindexer';
			$plugin_file = 'ps-postindexer/post-indexer.php';
			
			if(file_exists($plugin_dir)) {
				// Plugin ist installiert, aber möglicherweise nicht aktiviert
				$is_active = is_plugin_active_for_network($plugin_file);
				
				if(!$is_active) {
					// Erstelle Aktivierungslink mit Nonce
					$activate_url = wp_nonce_url(
						admin_url('network/plugins.php?action=activate&plugin=' . urlencode($plugin_file)),
						'activate-plugin_' . $plugin_file
					);
					echo '<div class="error"><p>'.__('<strong>Multisite Index wird benötigt.</strong> <a href="' . esc_url($activate_url) . '">Aktiviere Multisite-Index jetzt um MS Reader nutzen zu können</a>.', 'wmd_msreader').'</p></div>';
				}
			} else {
				// Plugin ist nicht installiert
				echo '<div class="error"><p>'.__('<strong>Multisite-Reader benötigt das Multisite Index-Plugin, um zu funktionieren.</strong> Du kannst dieses Plugin <a href="https://power-source.github.io/ps-postindexer/" target="blank">hier herunterladen.</a>.', 'wmd_msreader').'</p></div>';
			}
		}
	}
